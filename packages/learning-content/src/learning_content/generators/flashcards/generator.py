...
from learning_content.generators.flashcards.parser import FlashcardsParser
from learning_content.generators.flashcards.prompt_builder import (
    PROMPT_VERSION,
    FlashcardsPromptBuilder,
)
...
class FlashcardsGenerator(BaseLearningGenerator):
    """
    Concrete implementation for generating structured JSON flashcards.
    Delegates all orchestration to BaseLearningGenerator.
    """

    def __init__(self, provider: AbstractTextGenerationProvider) -> None:
        super().__init__(provider)
        self._prompt_builder = FlashcardsPromptBuilder()
        self._parser = FlashcardsParser()
    ...
    def info(self) -> GeneratorInfo:
        return GeneratorInfo(
            generator_id="flashcards-generator-v1",
            generator_name="Flashcards Generator",
            generator_version="1.0",
            provider_name=self._provider.info.provider_id,
            supported_content_types=(ContentType.FLASHCARDS,),
            maximum_chunks=150,
            maximum_tokens=8000,

...
export function ExperienceFlow() {
    ...
    async function restoreSession() {
      try {
        ...
        if (session && isMounted) {
          ...
          const savedStateStr = localStorage.getItem(`kogniq_workspace_state_${session.user.id}`);
          if (savedStateStr) {
            try {
  ...
  return <LayoutGroup id="kogniq-flow"><main className="min-h-dvh overflow-hidden bg-canvas"><AnimatePresence mode="sync" initial={false}>{phase === "arrival" && <Arrival onEnter={() => setPhase("access")} reduceMotion={reduceMotion} />}{phase === "access" && <Access onContinue={(uid) => { setSessionUserId(uid); setPhase("intention"); }} reduceMotion={reduceMotion} />}{phase === "intention" && <Intention activeIndex={activeIndex} onActiveChange={setActiveIndex} onSelect={selectEnvironment} reduceMotion={reduceMotion} />}{phase === "workspace" && <motion.div key="workspace" initial={false} animate={{ opacity: 1 }}><WorkspaceEngine initialEnvironmentId={chosenEnvironmentId} initialHistory={restoredHistory} initialMemory={restoredMemory} sessionUserId={sessionUserId} onLeave={() => setPhase("intention")} /></motion.div>}</AnimatePresence></main></LayoutGroup>;
}

function Arrival({ onEnter, reduceMotion }: { onEnter: () => void; reduceMotion: boolean | null }) {
  return <motion.section key="arrival" initial={false} exit={reduceMotion ? undefined : { opacity: 0.98 }} className="arrival-field relative grid min-h-dvh place-items-center overflow-hidden bg-[hsl(var(--night))] text-[hsl(var(--night-ink))]"><button onClick={onEnter} className="group relative px-8 py-6 text-left focus-visible:outline-offset-8" aria-label="Enter Kogniq"><motion.span layoutId="kogniq-word" initial={reduceMotion ? false : { filter: "blur(1.1px)", letterSpacing: "-0.035em", opacity: 0.9 }} animate={{ filter: "blur(0px)", letterSpacing: "-0.075em", opacity: 1 }} transition={{ duration: reduceMotion ? 0 : 1.05, ease: [0.2, 0.75, 0.25, 1] }} className="kogniq-word text-[clamp(3.2rem,10vw,7rem)]">Kogniq</motion.span><span aria-hidden className="ml-1 inline-block h-[.82em] w-px translate-y-[.08em] bg-current align-baseline opacity-70 [animation:locus-idle_2.2s_cubic-bezier(.45,0,.55,1)_infinite]" /></button></motion.section>;
}

function Access({ onContinue, reduceMotion }: { onContinue: (userId: string) => void; reduceMotion: boolean | null }) {
  ...
  const focusEmail = () => { setEmailFocused(true); setLoginError(null); };

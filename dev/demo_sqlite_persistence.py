...
async def main() -> None:
    ...
    block = NormalizedBlock(block_id="b1", block_type=BlockType.PARAGRAPH, text="Sample text", order=0)
    ...
    chunk = Chunk(
        ...
        chunk_index=0,
        text="Sample text",
        metadata=ChunkMetadata(processor='demo', document_version='v1', source='demo', checksum='123'),
        statistics=ChunkStatistics(character_count=10, line_count=1, word_count=2, estimated_tokens=2, processing_timestamp=datetime.now(UTC), confidence=1.0),
        created_at=datetime.now(UTC)
    )
    chunks = ChunkCollection(chunks=(chunk,))
    
    c1 = KnowledgeConcept(id="c1", title="SQLite", concept_type=ConceptType.UNKNOWN, description="", aliases=(), metadata=KnowledgeMetadata(source_document=doc_id, source_chunk="chunk-1", language="en", confidence=1.0, extraction_version="1.0", created_by="demo"))
    ...
    learning = LearningContent(
        ...
        metadata=LearningContentMetadata(provider="demo", model="demo", model_version="1", generation_version="1", language="en", educational_level="101", subject="demo", syllabus="demo", prompt_version="1", tags=("demo",)),
        statistics=LearningContentStatistics(character_count=10, word_count=2, estimated_tokens=2, processing_time_ms=10.0, confidence=1.0),
        created_at=datetime.now(UTC)
    )

...
def create_doc(blocks: list[NormalizedBlock]) -> NormalizedDocument:
    pages = (
        NormalizedPage(page_number=1, blocks=tuple(blocks)),
    )
...
def test_deeply_nested_heading_blocks() -> None:
    ...
    nested_layer_2 = NormalizedBlock(
        block_id="3", block_type=BlockType.PARAGRAPH, text="Wrapper 2", order=0, 
        children=(heading_block,)
    )
    nested_layer_1 = NormalizedBlock(
        block_id="2", block_type=BlockType.PARAGRAPH, text="Wrapper 1", order=0, 
        children=(nested_layer_2,)
    )
    
    blocks = [
        NormalizedBlock(
            block_id="1", block_type=BlockType.PARAGRAPH, text="Root", order=0, 

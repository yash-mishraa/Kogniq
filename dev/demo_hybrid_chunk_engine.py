...
def main() -> None:
    ...
    blocks_a = [
        NormalizedBlock(
            block_id="a1", block_type=BlockType.PARAGRAPH, 
            text="This is a document with structure.", order=0
        ),
        NormalizedBlock(
            block_id="a2", block_type=BlockType.HEADING, 
            text="Chapter 1", order=1
        ),
        NormalizedBlock(
            block_id="a3", block_type=BlockType.PARAGRAPH, 
            text="The first chapter discusses structural chunking.", order=2
        ),
    ]
    doc_a = NormalizedDocument(
        ...
        source="demo", checksum="1", version="1.0", created_at=datetime.now(UTC)
    )
    
    blocks_b = [
        NormalizedBlock(
            block_id="b1", block_type=BlockType.PARAGRAPH, 
            text=(
        ...
        NormalizedBlock(
            block_id="b3", block_type=BlockType.PARAGRAPH, 
            text=(
    ...
    doc_b = NormalizedDocument(
        ...
        source="demo", checksum="2", version="1.0", created_at=datetime.now(UTC)
    )

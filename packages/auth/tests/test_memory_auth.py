...
async def test_authentication_missing_user(provider: MemoryAuthenticationProvider) -> None:
    request = AuthenticationRequest(
        provider=AuthProvider.LOCAL,
        payload={"email": "unknown@kogniq.ai"}
    )
...
async def test_multiple_identities(
    ...
    # Login via Local
    res1 = await provider.authenticate(AuthenticationRequest(
        provider=AuthProvider.LOCAL,
        payload={"email": "test@kogniq.ai"}
    ))
    
    # Login via Google
    res2 = await provider.authenticate(AuthenticationRequest(
